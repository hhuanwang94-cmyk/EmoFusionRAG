...
@file: bm25.py
@time: 2022/04/16
@description: BM25 检索系统，支持关键词红色高亮显示
"""
...
# ===== 统一使用服务器实际路径 =====
_DATA_DIR = "/mnt/data4/WHH/MyEmoHH/BM25/data"
print(f"🔍 [BM25] 数据目录: {_DATA_DIR}")
...
class BM25(object):
    # ==================此处添加自己的数据位置=====================
    # 之前是硬编码的绝对路径 /root/autodl-tmp/myEmotionRoleRag/...
    # 现在改为基于当前文件目录的相对路径，适配 /EmotionHH/MyEmoHH/BM25/data
    _param_pkl = os.path.join(_DATA_DIR, "param.pkl")
    _docs_path = os.path.join(_DATA_DIR, "characters.txt")
    _stop_words_path = os.path.join(_DATA_DIR, "stop_words.txt")
    _stop_words = []
    ...
    def _build_param(self):
        ...
        # 保存参数
        os.makedirs(os.path.dirname(self._param_pkl), exist_ok=True)
    ...
    def highlight_text(self, text: str, keywords: list):
        """
        将文本中的关键词用红色标记
        :param text: 原始文本
        ...
        for word in sorted_keywords:
            if word in highlighted:
                highlighted = highlighted.replace(word, f"{RED}{word}{ENDC}")
        return highlighted

    def print_rank_with_highlight(self, query: str, top_k=10):
        """
        输出排序结果，并高亮关键词
        :param query: 查询文本
        :param top_k: 显示前 K 个结果
        """
        words = [word for word in jieba.lcut(query) if word and word not in self._stop_words]
        result = self.cal_similarity_rank(query)
        print(f"\n🔍 查询: {query}")
        print(f"🎯 关键词: {', '.join(words)}")
        print("="*50)

        for i, (doc, score) in enumerate(result[:top_k], 1):
            if score <= 0:
                break
            highlighted_doc = self.highlight_text(doc, words)
            print(f"{i:2d}. [Score: {score:.4f}] {highlighted_doc}")


if __name__ == '__main__':
    bm25 = BM25()
    query_content = "那褴褛少年见她伤了武娘子，又掳劫二女，大感不平，耳听得陆程二女惊呼，当即跃起，往李莫愁身上抱去。"
    print("**" * 20)
    bm25.print_rank_with_highlight(query_content, top_k=5)